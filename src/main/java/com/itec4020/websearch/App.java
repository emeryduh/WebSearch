...
public class App {
	...
	public App() {
    	...
    	try {
			...
			search("foreign minorities, Germany", "What language and cultural differences impede the integration" + 
	...
	public void search(String title, String content) {
		try {
			...
			rest.performRequestAsync(request,
	...
	/**
	 * TODO: Remove indexBump and replace with a more dynamic way to generate indicies
	 * 
	...
	public void indexDocuments(String folder, int indexBump) {
		...
		// CountDownLoatch for asynchronously requests to be used to identify when the next step can start
	...
	/**
	 * Breaks apart an entire document that follows the format of
	 * the decompressed files into separate JSON strings.  The strings
	 * will be used into ElasticSearch.
	 * 
	...
	public ArrayList<String> getJSONsFromPath(String path) {
		...
		try {
			...
			// This occurs when there are no more DOC tags to select, in which case return the JSONs
			return jsons;
	...
	/**
	 * Extracts data from the given Element into the JSON format.  It is
	 * expected that the Element provided was generated from Jsoup pulling out a 
	 * single DOC tag.  (i.e. the root element must have no siblings and must be a
	 * DOC tag)
	 * 
